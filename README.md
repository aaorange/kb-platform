...
> 基于 RAG 的企业级知识库智能问答平台，核心特色是**四维细粒度权限鉴权**（全局 / 部门 / 角色 / 个人）：权限过滤下沉到 Milvus 检索层，无权内容从源头不可见；融合 LangGraph 查询引擎（实体对齐 → 混合检索 → 重排 → 生成）、引用溯源、FAQ 自动沉淀与知识缺口挖掘，覆盖"文档多源维护 → 权限管控 → AI 鉴权问答 → 数据运营"全链路。
